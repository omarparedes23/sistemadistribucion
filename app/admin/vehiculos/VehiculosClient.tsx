"use client";

import { useState, useActionState } from "react";
import {
  createVehicle,
  updateVehicle,
  toggleVehicleActive,
  type VehicleState,
} from "@/lib/actions/vehicles";
import {
  EntityTable,
  EntityTableRow,
  EntityTableCell,
} from "../_components/EntityTable";
import { Pencil, Power, PowerOff, Plus } from "lucide-react";

type Vehicle = {
  id: string;
  plate_number: string;
  branch_id: string;
  capacity_kg: number | null;
  capacity_m3: number | null;
  active: boolean;
  re_branches: { name: string } | null;
};

type Branch = { id: string; name: string };

export function VehiculosClient({
  initialVehicles,
  branches,
}: {
  initialVehicles: Vehicle[];
  branches: Branch[];
}) {
  const [editingId, setEditingId] = useState<string | null>(null);

  const [formState, formAction, isPending] = useActionState<
    VehicleState | null,
    FormData
  >(async (_prev, formData) => {
    const id = formData.get("id") as string;
    const result = id
      ? await updateVehicle(null, formData)
      : await createVehicle(null, formData);
    if (result.success) window.location.reload();
    return result;
  }, null);

  const toggleStateTuple = useActionState<
    VehicleState | null,
    FormData
  >(async (_prev, formData) => {
    const result = await toggleVehicleActive(null, formData);
    if (result.success) window.location.reload();
    return result;
  }, null);
  const toggleAction = toggleStateTuple[1];
  const isTogglePending = toggleStateTuple[2];

  const editingVehicle = initialVehicles.find((v) => v.id === editingId);

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Vehículos</h1>
      </div>

      <form
        key={editingId ?? "new"}
        action={formAction}
        className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm"
      >
        <input type="hidden" name="id" value={editingId ?? ""} />

        {formState?.success === false && (
          <div className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">
            {formState.error}
          </div>
        )}

        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-1.5">
            <label htmlFor="plate_number" className="text-sm font-medium text-gray-700">
              Placa <span className="text-red-500">*</span>
            </label>
            <input
              id="plate_number"
              name="plate_number"
              type="text"
              defaultValue={editingVehicle?.plate_number ?? ""}
              required
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              placeholder="Ej. ABC-123"
            />
          </div>

          <div className="space-y-1.5">
            <label htmlFor="branch_id" className="text-sm font-medium text-gray-700">
              Sucursal <span className="text-red-500">*</span>
            </label>
            <select
              id="branch_id"
              name="branch_id"
              defaultValue={editingVehicle?.branch_id ?? ""}
              required
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
            >
              <option value="">Selecciona una sucursal</option>
              {branches.map((b) => (
                <option key={b.id} value={b.id}>
                  {b.name}
                </option>
              ))}
            </select>
          </div>

          <div className="space-y-1.5">
            <label htmlFor="capacity_kg" className="text-sm font-medium text-gray-700">
              Capacidad (kg)
            </label>
            <input
              id="capacity_kg"
              name="capacity_kg"
              type="number"
              step="0.01"
              min="0"
              defaultValue={editingVehicle?.capacity_kg ?? ""}
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              placeholder="Ej. 5000"
            />
          </div>

          <div className="space-y-1.5">
            <label htmlFor="capacity_m3" className="text-sm font-medium text-gray-700">
              Capacidad (m³)
            </label>
            <input
              id="capacity_m3"
              name="capacity_m3"
              type="number"
              step="0.0001"
              min="0"
              defaultValue={editingVehicle?.capacity_m3 ?? ""}
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              placeholder="Ej. 15"
            />
          </div>
        </div>

        <div className="mt-4 flex items-center gap-3">
          <button
            type="submit"
            disabled={isPending}
            className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-700 disabled:opacity-50"
          >
            <Plus className="h-4 w-4" />
            {isPending
              ? "Guardando..."
              : editingId
              ? "Actualizar vehículo"
              : "Crear vehículo"}
          </button>
          {editingId && (
            <button
              type="button"
              onClick={() => setEditingId(null)}
              className="text-sm text-gray-500 hover:text-gray-700"
            >
              Cancelar edición
            </button>
          )}
        </div>
      </form>

      <EntityTable
        columns={[
          { header: "Placa" },
          { header: "Sucursal" },
          { header: "Capacidad" },
          { header: "Estado" },
          { header: "Acciones", className: "w-40" },
        ]}
        caption="Lista de vehículos"
      >
        {initialVehicles.map((vehicle) => (
          <EntityTableRow key={vehicle.id}>
            <EntityTableCell className="font-medium text-gray-900">
              {vehicle.plate_number}
            </EntityTableCell>
            <EntityTableCell className="text-gray-500">
              {vehicle.re_branches?.name ?? "—"}
            </EntityTableCell>
            <EntityTableCell className="text-gray-500">
              <div className="space-y-0.5 text-xs">
                {vehicle.capacity_kg != null && <div>{vehicle.capacity_kg} kg</div>}
                {vehicle.capacity_m3 != null && <div>{vehicle.capacity_m3} m³</div>}
                {vehicle.capacity_kg == null && vehicle.capacity_m3 == null && "—"}
              </div>
            </EntityTableCell>
            <EntityTableCell>
              <span
                className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                  vehicle.active
                    ? "bg-green-50 text-green-700"
                    : "bg-gray-100 text-gray-600"
                }`}
              >
                {vehicle.active ? "Activo" : "Inactivo"}
              </span>
            </EntityTableCell>
            <EntityTableCell>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setEditingId(vehicle.id)}
                  className="rounded p-1.5 text-gray-500 transition hover:bg-gray-100 hover:text-indigo-600"
                  title="Editar"
                >
                  <Pencil className="h-4 w-4" />
                </button>
                <form action={toggleAction} className="inline">
                  <input type="hidden" name="id" value={vehicle.id} />
                  <input type="hidden" name="active" value={String(vehicle.active)} />
                  <button
                    type="submit"
                    disabled={isTogglePending}
                    className={`rounded p-1.5 transition hover:bg-gray-100 ${
                      vehicle.active
                        ? "text-gray-500 hover:text-red-600"
                        : "text-gray-500 hover:text-green-600"
                    }`}
                    title={vehicle.active ? "Desactivar" : "Activar"}
                  >
                    {vehicle.active ? (
                      <PowerOff className="h-4 w-4" />
                    ) : (
                      <Power className="h-4 w-4" />
                    )}
                  </button>
                </form>
              </div>
            </EntityTableCell>
          </EntityTableRow>
        ))}
        {initialVehicles.length === 0 && (
          <EntityTableRow>
            <EntityTableCell colSpan={5} className="py-8 text-center text-gray-500">
              No hay vehículos registrados.
            </EntityTableCell>
          </EntityTableRow>
        )}
      </EntityTable>
    </div>
  );
}
